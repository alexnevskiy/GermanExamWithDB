...
На данном этапе нужно разрешить обмен данными между фрагментами, используя `ViewModel`. Нашей задачей является соединение фрагментов с `ViewModel`, чтобы при изменении одного `SeekBar` обновлялся другой `SeekBar`.
